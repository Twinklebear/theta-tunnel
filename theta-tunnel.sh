...
if [ "$1" == "-h" ]; then
	...
	printf "\tto a connection on Theta compute node nid0<worker id>:<port>\n"
...
ssh -L $PORT:localhost:$PORT ${USER}@theta.alcf.anl.gov <<-1
	...
	ssh -L "$PORT":localhost:"$PORT" thetamom1 <<-2
		echo "Opening socat to nid0${WORKER_ID}:${PORT} ..."
		socat TCP-LISTEN:$PORT,reuseaddr TCP:nid0${WORKER_ID}:$PORT
	2
1
